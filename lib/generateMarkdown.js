    ...
    if (defined(schemaRef)) {
        if (schemaRef === 'http://json-schema.org/draft-03/schema') {
            ...
            if ((!options.suppressWarnings) &&
                ...
                schemaRef !== 'https://json-schema.org/draft/2019-09/schema' &&
...
function getTableOfContentsMarkdown(schema, orderedTypes, headerLevel) {
    var md = style.getHeaderMarkdown(headerLevel) + ' Schemas\n';
    for (var title in orderedTypes) {
        ...
        if (title === schema.title || currentType.parents.length > 1 || currentType.parents.indexOf(schema.title) !== -1) {
            ...
            const item = style.getTOCLink(title, typeName);
...
function getSchemaMarkdown(schema, fileName, headerLevel, suppressWarnings, schemaRelativeBasePath, knownTypes, autoLink, embedMode) {
    ...
    if (schemaType === 'object') {
        ...
        md += createPropertyDetail(schema, null, schema, schema.title, headerLevel, knownTypes, autoLink);
    }
...
function createPropertiesSummary(schema, knownTypes, autoLink) {
    ...
    var properties = schema.properties || schema.patternProperties;
        ...
        for (var name in properties) {
            if (properties.hasOwnProperty(name)) {
                ...
                md += style.addTableRow([
                  ...
                  + (defined(summary.default) ? `<br><br>default:<br>${summary.default}` : '')
                  + (defined(summary.const) ? `<br><br>must be:<br>${summary.const}` : '')
                ]);
            }
        }
...
function createPropertyDetail(property, name, schema, title, headerLevel, knownTypes, autoLink) {
    ...
    } else if (defined(summary.description) && defined(name)) {
    ...
    if (defined(name)) md += style.bulletItem(style.propertyDetails('Required') + ': ' + summary.required, 0);

    if (defined(summary.default)) md += style.bulletItem(style.propertyDetails('Default') + ': ' + summary.default, 0);
...
function createPropertiesDetails(schema, title, headerLevel, knownTypes, autoLink) {
    ...
    for (var name in properties) {
        if (properties.hasOwnProperty(name)) {
            md += createPropertyDetail(properties[name], name, schema, title, headerLevel, knownTypes, autoLink)
        }
    }
    md += '\n';
...
function getPropertySummary(property, knownTypes, autoLink) {
    ...
    if (type === 'any' && property.description && property.description.startsWith('`func ')) type = 'func';

    if (type === 'string' && defined(property.enum)) type = 'enum';
    ...
    if (type === 'object' || type === 'any') {
        ...
        if (defined(property.anyOf)) types = {name: 'anyOf', delim: '&#124;', values: property.anyOf};

        if (defined(property.oneOf))  types = {name: 'oneOf', delim: '&#124;', values: property.oneOf};

        if (defined(property.allOf)) types = {name: 'allOf', delim: '&#124;', values: property.allOf};

        if (defined(types)) {
          type = types.name;
          formattedType = types.values.map(t => {
              return style.linkType(style.typeValue(t.typeName || t.type), t, autoLink);
          }).join(types.delim);
        }
    }
    ...
    if (defined(propertyDefault)) {
        ...
        if (_.isString(defaultString)) {
            defaultString = defaultString.replaceAll(/^(\.\.\/)+_wk\/schemas\//g, '');
        }

        defaultString = style.defaultValue(defaultString, type);
    }

    var constValue = property.const;

    if (defined(constValue)) constValue = style.defaultValue(constValue, type);
...
* @param  {Object[]} knownTypes  Array of known strings that are types that should be auto-linked if found.
* If there are multiple types with the same starting root string, it's imperative that the array is sorted such that the longer
*   names are ordered first.
* @param  {string} autoLink Enum value indicating how the auto-linking should be handled.
