...
* @param  {Object} type - The type whose reference within string should be linked.
...
function linkType(string, type, autoLink) {
    type = _.isObject(type) ? _.get(type, 'schema.typeName') : type;
    ...
    if (type === 'integer' || type === 'string' ||
        ...
        type === 'func'    || type === 'anyOf'  ||
        type === 'allOf'   || type === 'oneOf'
    ) return string;
    ...
    string = string.replaceAll('`' + type + '`', getLinkMarkdown(styleCode(type), typeLink));

    return string.replaceAll(
      new RegExp('`' + type + '#/(.+?)`', 'g'),
      getLinkMarkdown(styleCode(type), typeLink) + "`#/$1`");
}
