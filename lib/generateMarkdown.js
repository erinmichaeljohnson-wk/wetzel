        ...
        for (var name in properties) {
            if (properties.hasOwnProperty(name)) {
                ...
                md += style.addTableRow([
                  ...
                  + (defined(summary.default) ? `, default:<br>${summary.default}` : '')
                ]);
            }
        }
...
function getPropertySummary(property, knownTypes, autoLink) {
    ...
    if (type === 'object') {
        ...
        if (defined(property.anyOf)) types = {name: 'anyOf', delim: 'OR', values: property.anyOf};

        if (defined(property.oneOf))  types = {name: 'oneOf', delim: 'OR', values: property.oneOf};

        if (defined(property.allOf)) types = {name: 'allOf', delim: 'AND', values: property.allOf};

        if (defined(types)) {
          type = types.name;
          formattedType = types.values.map(t => {
              return style.linkType(style.typeValue(t.typeName || t.type), t, autoLink);
          }).join(`<br>${types.delim}<br>`);
        }
    }
